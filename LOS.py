...
def create_obsMapForLOS(mapData):
    grid = np.reshape(copy.deepcopy(mapData.data), (mapData.info.width,mapData.info.height))
    matrix_LOS =  convolve2d(grid, np.ones([2,2]), mode='same', boundary='fill', fillvalue=1)
    abs_mat = np.abs(grid)
    matrix_obsmap = (convolve2d(np.abs(abs_mat), np.ones([1, 1]), mode='same', boundary='fill', fillvalue=1)>0)

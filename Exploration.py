...
#import buildGraph
from ChoseExplorationPoint import ChoseExplorationPointClass
...
class ExplorationCalss:
    ...
    def route_to_best_frontier(self):
        currentTarget, path = self.chose_mission.do_step(self.graph, current_frontiers)
        self.route_to_point = currentTarget
        return path

    def get_covrage(self):
        if self.map is None:
            return 0

        size_of_world = self.map.map_sizeX * self.map.map_sizeX
